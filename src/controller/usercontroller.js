...
const signup = async (req, res) => {
  ...
  if (isEXist)
    ...
    try {
      ...
      return res
        .status(200)
        .json({ success: true, data: "User Registred Successfully" });
    } catch (error) {
  ...
  try {
    ...
    if (isExist) {
      ...
      if (ispasswordcorrect) {
        ...
        res
          .status(200)
          .json({ success: true, user: isExist, token: accesstoken });
      } else res.status(200).json({ success: false, data: "Invalid Password" });
    } else
